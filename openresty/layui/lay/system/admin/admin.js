...
layui.use(['layer', 'element'], function() {
	...
	var parentItems = $('.layui-side .layui-nav-myitem li dt');
	var parentItemsIsClick = false;
	$(parentItems).click(function() {
		var that = this;
	...
	function clearOtherCss(parentItems) {
		parentItems.children('.layui-icon-triangle-r').css({
			"transform": "rotate(0deg)"
		});
	...
	//左侧菜单栏添加事件(子菜单事件)
	$(items.parent("dd")).click(function() {
	...
	function loginOut() {
		...
		$.ajax({
			url: IP + '/api/blog-oauth2/oauth2/loginout?access_token=' + access_token,
			async: false,
			headers: {
				'Authorization': localStorage.getItem("access_token") == null ? "" : "Bearer " + localStorage.getItem("access_token")
			},
			...
			success: function(result, status, xhr) {
				...
				layer.closeAll('loading');
				window.parent.location.href = IP + '/admin/login.html';
			}
		});
	}
});